<small class="d-inline-block">
  ...
  @unless($house && $house->is_rented)
    <button id="_mfa_a_{{$loop->iteration}}" class="fa fa-check mx-1 py-0 px-2 btn btn-light" style="color: green;"
            onclick="MFA.approve({{$house->getActionParams($user, 'approve')}})">
    </button>
    <button id="_mfa_d_{{$loop->iteration}}" class="fa fa-times mx-1 py-0 px-2 btn btn-light" style="color: red;"
...
@if($index === 0)
  @push('actions-scripts')
    <script src="{{ asset('js/modal-scripts.blade.php') }}" defer></script>
  @endpush
@endif--}}
